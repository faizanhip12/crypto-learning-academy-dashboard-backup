...
const VideoUploader = ({
  ...
  const { getRootProps, getInputProps } = useDropzone({
    ...
    onDropRejected: () => {
      toast.error(`You can only upload ${maxFiles} file & maximum size of ${5} GB.`, {
        duration: 2000
      })
    }
  })
  ...
  const fileList = files.map((file: FileProp | any) => (
    <ListItem key={file.name}>
      <div className='file-details'>
        ...
        <div>
          <Typography className='file-name'>{textOverflow(file.name, 30)}</Typography>
      ...
      <Card>
        ...
        >
          <div {...getRootProps({ className: 'dropzone' })}>
            ...
            <Box
              sx={{ display: 'block', textAlign: 'center', color: theme.palette.customColors.grey, marginTop: '10px' }}
            >
